...
const income_values_schema = (await import("../../src/models/income_statement_values.model.js")).default;
...
describe("income_service", () => {
  ...
  test("Positive: filters and joins correctly", async () => {
    ...
    const value_rows = [
        { IncomeID: 1, FileID: 100, ApplicationID: 10, Value: converted_decimal },
    ];

    const key_rows = [
        { IncomeID: 1, Metric: "Revenue", Unit: "$" },
    ];

    mockFindLean(income_values_schema, value_rows);
    mockFindLean(income_schema, key_rows);
    ...
    expect(result).toEqual([
        ...
        ApplicationID: 10,
        Timeline: "2025-Q4",
        Value: 123.45, // correctly converted from Decimal128
        },
    ]);
    });
  ...
  test("Negative: no matching values → empty array", async () => {
    ...
    mockFindLean(income_values_schema, []);
    mockFindLean(income_schema, [{ IncomeID: 999, Metric: "Revenue", Unit: "$" }]);
  ...
  test("Negative: no matching keys → empty array", async () => {
    ...
    mockFindLean(income_values_schema, [{ IncomeID: 1, FileID: 10, ApplicationID: 1, Value: 50 }]);
    mockFindLean(income_schema, []); // no matching key rows
