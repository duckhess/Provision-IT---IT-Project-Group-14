...
const financial_values_schema = (await import("../../src/models/financial_statements_values.js"))
  .default;
...
describe("filter_statements service", () => {
  ...
  test("Positive: filters applied, joins financial + values + timeline correctly", async () => {
    ...
    const values = [{ FinancialID: 1, ApplicationID: 2, FileID: 3, Value: 1134824 }];
    const documents = [{ FinancialID: 1, Metric: "Revenue", Unit: "$" }];
    const timeline_map = new Map([[3, 2025]]);

    // mock DB + timeline service
    const value_found = mock_find_lean(financial_values_schema, values);
    const docs_found = mock_find_lean(financial_schema, documents);
  ...
  test("Negative: no matching values → empty array", async () => {
    ...
    mock_find_lean(financial_values_schema, []); // no values
    mock_find_lean(financial_schema, [{ FinancialID: 999, Metric: "Expenses", Unit: "$" }]);
  ...
  test("Edge case: no fileIDs → get_period returns empty map", async () => {
    ...
    const values = [{ FinancialID: 50, ApplicationID: 7, FileID: 100, Value: "999.99" }];
    const documents = [{ FinancialID: 50, Metric: "Profit Margin", Unit: "%" }];
