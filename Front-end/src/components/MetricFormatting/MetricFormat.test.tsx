...
// Flexible x-value extractor
function getRawXValue(rawItem: any): string | number {
  const possibleKeys = ["Timeline", "Period", "Year", "Date", "Benchmark", "Category"];
  for (const key of possibleKeys) {
    if (key in rawItem) return rawItem[key];
  }
  return 0;
}
...
describe("Data Transform Functions", () => {
  ...
  it("transformCovenants correctly converts covenant data", () => {
    const transformedKeyRatios: Dataset[] = transformTimelineMetricsPerCompany(
      "key_ratios",
      keyRatios
    );

    const datasets = transformCovenants(covenants, transformedKeyRatios);
    expect(datasets).toBeInstanceOf(Array);
    datasets.forEach(ds => {
      expect(ds.data).toBeInstanceOf(Array);
