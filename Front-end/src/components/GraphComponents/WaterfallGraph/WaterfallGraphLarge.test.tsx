...
// --- Tests ---
describe("WaterfallGraphLarge", () => {
  it("renders the chart title", () => {
    render(<WaterfallGraphLarge mergedSets={mockMergedSets} datasets={mockDatasets} title="Key Ratios" />);
  ...
  it("renders cells for each data entry", () => {
    render(<WaterfallGraphLarge datasets={mockDatasets} mergedSets={mockMergedSets} title="Key Ratios" />);
    const greenCells = screen.getAllByTestId(/mock-cell-#/); 
    expect(greenCells.length).toBeGreaterThan(0);
  });

  it("renders correctly and matches snapshot", () => {
    const { container } = render(<WaterfallGraphLarge datasets={mockDatasets} mergedSets={mockMergedSets} title="Key Ratios" />);
